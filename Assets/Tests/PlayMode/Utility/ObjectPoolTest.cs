...
namespace Tests
{
    public class ObjectPoolTest
    {
        ...
        public IEnumerator GetNReturn()
        {
            ...
            Assert.AreEqual(config.initialize_initializeCount - 1, objectPool.DeactiveCount);
        ...
        public IEnumerator CreateNGetNReturnAction()
        {
            ...
            for(int i = 0; i< config.initialize_initializeCount + 1;i++)
            {
                objectPool.Get();
            }
            Assert.AreEqual(config.initialize_initializeCount + 1, objectPool.Count);
        ...
        public IEnumerator ReturnAll()
        {
            ...
            Assert.AreEqual(objectPool.Count, config.initialize_initializeCount);
            Assert.AreEqual(objectPool.DeactiveCount, 0);
            Assert.AreEqual(objectPool.ActiveCount, config.initialize_initializeCount);

            objectPool.ReturnAll();

            Assert.AreEqual(objectPool.Count, config.initialize_initializeCount);
            Assert.AreEqual(objectPool.DeactiveCount, config.initialize_initializeCount);
